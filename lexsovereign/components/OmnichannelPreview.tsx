...
const OmnichannelPreview: React.FC = () => {
  ...
  const rawResponse = "The property transfer for Kofi Mensah regarding the plot in East Legon (GH-122) has been approved for $45,000. Expected completion is June 15th.";
  ...
  const runDiscovery = async () => {
    ...
    setTimeout(() => {
      addLog("OIDC Federated Lookup: Resolving Azure AD for 'Accra Partners'");
      addLog("Trust Chain Verified: GBA-ROOT-CA-01 certificate pinned.");
    }, 1400);
    ...
    setTimeout(() => {
      addLog("HSM Signing: JWT (RS256) generated in GH-ACC-HSM enclave.");
          ...
          {viewMode === 'simulator' ? (
            <div className="w-full max-w-[300px] h-[580px] bg-slate-900 rounded-[3rem] border-8 border-slate-800 shadow-2xl relative overflow-hidden flex flex-col scale-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-green-200 flex items-center gap-1 opacity-80">
                        <ShieldCheck size={10} /> Verified Node: GH-ACC-01
                      </p>
                    </div>
                  </div>
               </div>
               ...
               <div className="flex-1 bg-[#e5ddd5] p-3 space-y-4 overflow-y-auto pattern-dots">
                  <div className="bg-white p-3 rounded-2xl text-[11px] max-w-[85%] shadow-sm leading-relaxed text-slate-800">
                     I need a quick update on my property transfer in East Legon.
            ...
            <div className="w-full bg-slate-900 border border-slate-800 rounded-3xl p-6 h-[580px] space-y-8">
               <div className="text-center space-y-2">
                  ...
                  <p className="text-[10px] text-slate-500 font-mono">ENCLAVE: GH-ACC-HSM-A1</p>
               </div>
...
const TabBtn = ({ label, active, icon, onClick }: any) => (
  <button 
    ...
    className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all flex items-center gap-2 border ${
      active ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-slate-800/50 border-slate-700 text-slate-500 hover:text-slate-300'
    }`}
  >
    {icon}
    {label}

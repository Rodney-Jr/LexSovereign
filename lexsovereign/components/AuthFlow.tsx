...
const AuthFlow: React.FC<AuthFlowProps> = ({ onAuthenticated, onStartOnboarding, onSecretTrigger }) => {
  ...
  const finalizeAuth = () => {
    if (rememberMe) {
      localStorage.setItem('lexSovereign_session', JSON.stringify({ role: selectedRole, domain }));
    }
                ...
                <div className="py-10 flex justify-center">
                   <div 
                    ...
                    className={`w-32 h-32 rounded-[2.5rem] flex items-center justify-center border-4 cursor-pointer transition-all duration-700 relative group ${
                      mfaStatus === 'VERIFIED' ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400 shadow-[0_0_50px_rgba(16,185,129,0.3)] scale-110' :

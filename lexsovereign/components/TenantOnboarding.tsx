...
const TenantOnboarding: React.FC<{ onComplete: (mode: AppMode) => void }> = ({ onComplete }) => {
  ...
  const isStep1Valid = formData.name && entityType !== null;
        ...
        {steps.map((s) => (
          <React.Fragment key={s.id}>
            <div className={`flex flex-col items-center gap-2 transition-all ${step >= s.id ? 'text-emerald-400' : 'text-slate-600'}`}>
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border-2 font-bold text-sm transition-all duration-500 ${
                step === s.id ? 'bg-emerald-500/20 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.4)] rotate-3' : 
          ...
          {step === 1 && (
            <div className="space-y-10 animate-in slide-in-from-right-4 duration-500 flex-1">
              ...
              <div className="grid grid-cols-2 gap-8 pt-4">
                ...
                <div className="space-y-2.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-1">OIDC Domain Handshake</label>
                  <div className="relative">
                    <Fingerprint className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-600" size={18} />
                    <input 
                      className="w-full bg-slate-950 border border-slate-800 rounded-[1.5rem] pl-14 pr-6 py-4 text-xs font-mono text-blue-400 focus:outline-none"
                      value={formData.idpUrl}
                      onChange={e => setFormData({...formData, idpUrl: e.target.value})}
                    />
                  </div>
                </div>
              </div>
...
const PlanCard = ({ plan, active, onClick, price, desc, features, highlight }: any) => (
  <button 
    onClick={onClick}
    className={`p-6 rounded-[2rem] border-2 transition-all text-left flex flex-col relative ${
      active ? 'bg-emerald-500/10 border-emerald-500 shadow-2xl shadow-emerald-500/10' : 'bg-slate-950 border-slate-800 hover:border-slate-700'
...
const RouteCard = ({ active, onClick, icon, title, desc, color, sub }: any) => (
  <button 
    onClick={onClick}
    className={`p-8 rounded-[2.5rem] border-2 transition-all text-left flex flex-col justify-between group h-64 ${
      active ? `bg-${color}-500/10 border-${color}-500 shadow-2xl shadow-${color}-500/10` : 'bg-slate-950 border-slate-800 hover:border-slate-700'
    }`}
  >

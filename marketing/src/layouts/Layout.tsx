        ...
        <div className="min-h-screen bg-slate-950 text-slate-50 font-inter selection:bg-indigo-500/30">
            ...
            >
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center gap-8">
                        ...
                        {String(import.meta.env.VITE_SHOW_PRICING).toLowerCase() === 'true' && <NavLink to="/pricing">Pricing</NavLink>}
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden absolute top-full left-0 right-0 bg-slate-950 border-b border-slate-800 p-6 flex flex-col gap-4 shadow-2xl">
                        ...
                        {String(import.meta.env.VITE_SHOW_PRICING).toLowerCase() === 'true' && <MobileNavLink to="/pricing">Pricing</MobileNavLink>}
            ...
            <footer className="bg-slate-950 border-t border-slate-900 pt-16 pb-8">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
                        ...
                        <div>
                            ...
                            <ul className="space-y-3 text-sm text-slate-400">
                                ...
                                {String(import.meta.env.VITE_SHOW_PRICING).toLowerCase() === 'true' && <li><Link to="/pricing" className="hover:text-indigo-400 transition-colors">Pricing</Link></li>}
                            </ul>
                        </div>
                    ...
                    <div className="border-t border-slate-900 pt-8 flex flex-col items-center md:items-start text-xs text-slate-500 gap-6">
                        <div className="flex flex-col md:flex-row justify-between w-full items-center md:items-start gap-4">
                            <div className="text-center md:text-left">
                                <p className="font-semibold text-slate-300">© 2026 Nexus Technologies Limited (Reg: CS339712014) trading as NomosDesk.</p>
                                <p className="text-slate-500 mt-1">Registered in Accra, Ghana.</p>
                            </div>

                            <div className="flex gap-6 mt-4 md:mt-0 items-center">
                                <Link to="/privacy" className="hover:text-slate-300 font-medium transition-colors">Privacy Policy</Link>
                                <Link to="/terms" className="hover:text-slate-300 font-medium transition-colors">Terms of Service</Link>
                            </div>
                        </div>

                        {/* Lean Platform Disclaimer */}
                        <div className="w-full p-4 bg-slate-900/40 rounded-xl border border-slate-800/60 mt-2">
                            <p className="leading-relaxed text-[11px] text-slate-400/80 text-center md:text-left">
                                NomosDesk provides workflow software, document automation, and AI assistance designed exclusively for legal professionals. <strong className="font-medium text-slate-300">NomosDesk is a technology platform, not a law firm.</strong> We do not provide legal advice, legal opinions, or representation. Your use of this website, the NomosDesk application, or its tools does not establish an attorney-client relationship. All content generated by our AI features must be independently verified by a licensed attorney prior to use. Legal professionals maintain full, independent responsibility for their client matters and ethical obligations.
                            </p>
                        </div>
                    </div>
                </div>
